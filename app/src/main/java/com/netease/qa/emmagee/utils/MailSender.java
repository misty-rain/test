...
public class MailSender {
	...
	public static boolean sendTextMail(String sender, String encryptPassword, String smtp, String subject, String content, String file,
		...
		if (maillists == null || maillists.length == 0 || ("".equals(maillists[0].trim()))) {
			...
			// Get session
			props.put("mail.smtp.auth", "false"); // 如果需要密码验证，把这里的false改成true
			...
			try {
				...
				// 创建邮件发送者地址
				Address from = new InternetAddress("taowu78@gmail.com");
				// 设置邮件消息的发送者
				mailMessage.setFrom(from);
				// 创建邮件的接收者地址，并设置到邮件消息中
				//for (int i = 0; i < maillists.length; i++) {
					// Message.RecipientType.TO属性表示接收者的类型为TO
					mailMessage.addRecipient(Message.RecipientType.TO, new InternetAddress("wtaoworld@163.com"));
				//}
